...
def get_relativedates(scans, maximal_period=96):
    ...
    normalized_dates = normalized_dates / maximal_period * 2 - 1
...
if __name__=='__main__':
    ...
    Generator = Conv3DLSTM(input_dim=2,hidden_dim=[64,64,64],kernel_size=[(3,3,3),(3,3,3),(3,3,3)],num_layers=3,\
    ...
    for patient in patientlist:
        ...
        ## all patients
        combs = itertools.combinations(range(len(scans)),3)
        for comb in combs:
            scan_save_dir = os.path.join(save_dir, patient+'_'+scans[comb[0]] + '_'+scans[comb[1]]+'_'+scans[comb[2]])
            if not os.path.isdir(scan_save_dir):
                os.mkdir(scan_save_dir)
            input_imgs = torch.concat([imgs[comb[0],:].unsqueeze(0),imgs[comb[1],:].unsqueeze(0)],dim=0)
            input_segs = torch.concat([segs[comb[0],:].unsqueeze(0),segs[comb[1],:].unsqueeze(0)],dim=0)
            input_data = torch.concat([input_imgs.unsqueeze(1),input_segs.unsqueeze(1)], dim=1)
            input_data = input_data.unsqueeze(0)
            with torch.no_grad():
                pred_img, pred_seg, state = Generator(input_data, None)
                for j in range(pred_seg.shape[1]):
                    ...
                    save_name = os.path.join(scan_save_dir, patient + "_" + scans[comb[j+1]]+".nii.gz")
                    nib.save(img_to_save,save_name)
            
                    mask_save_name = os.path.join(scan_save_dir, patient + "_" + scans[comb[j+1]]+"_seg.nii.gz")
