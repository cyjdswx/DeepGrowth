...
from models.networks.encoder import TensorEncoderLSTM,TensorDownX4EncoderLSTMNot, TensorDownX4EncoderLSTMold,TensorDownEncoderLSTMET, TensorDownX4EncoderLSTM, TensorDownEncoderLSTMNot, TensorDownX8EncoderLSTM
...
if __name__=='__main__':
    ...
    #Encoder = TensorDownX4EncoderLSTMNot(num_in=3,dim=32, hidden_dim=32,n_blocks=5)
    Encoder.cuda()
    ...
    for patient in patientlist:
        ...
        d, h, w = patch_size
        with torch.no_grad():
            #features = Encoder(input_pcl, study_dates)
            print('input',input_tensor.shape)
            print(time_interval)
            features_ori, features_lstm = Encoder(input_tensor, time_interval)
            #features = torch.concat([features_ori, features_lstm[:,-1,:].unsqueeze(1)],dim=1)
            ...
            for j in range(generated_sdf.shape[1]):
                ...
                #print('sum', torch.sum((mask < 0).float()))
