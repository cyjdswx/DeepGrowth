import sys,os
from data.mrisdf_dataset import LongMriFullDatasetThree, LongMriFullDataset, LongMriFullDatasetMore
from models.Multimrisdf_model import MultimrisdfModel
import torch
import time
...
if __name__=='__main__':
    ...
    #parser.add_argument('--dataset_dir', type=str, default='/exports/lkeb-hpc/ychen/01_data/08_VS_followup/longitudinal_t1ce_crossval_more_75p')
    parser.add_argument('--dataset_dir', type=str, default='/exports/lkeb-hpc/ychen/01_data/08_VS_followup/longitudinal_t1ce_crossval_more_largerbbox')
    ...
    parser.add_argument('--patch_height', type=int, default=80, help='# of height of image patch')
    parser.add_argument('--patch_width', type=int, default=80, help='# of width of image patch')
    parser.add_argument('--patch_thickness', type=int, default=80, help='# of slices of image patch')
    ...
    parser.add_argument('--pretrained', type=str,help='test')
    ...
    parser.add_argument('--niter_decay', type=int, default=151, help='# of iter to linearly decay learning rate to zero')
    ...
    parser.add_argument('--E_lr', type=float, default=0.0001, help='initial learning rate for adam')
    ...
    train_instance = LongMriFullDatasetThree(data_root, listfile, (patch_depth, patch_height, patch_width), is_train=True)
    #train_instance = LongMriFullDatasetDiff(data_root, listfile, (patch_depth, patch_height, patch_width), is_train=True)
    ...
    val_instance = LongMriFullDatasetThree(data_root, listfile, (patch_depth, patch_height, patch_width), is_train=False)
    #val_instance = LongMriFullDatasetDiff(data_root, listfile, (patch_depth, patch_height, patch_width), is_train=False)
    print("dataset [%s] of size %d was created" %
    ...
    # initialize trainer
    #model = MultiMriinrlstmModel(opt, device) 
    #model = MultiMripntV2Model(opt, device) 
    model = MultimrisdfModel(opt, device) 
    #model = MultimrisdfdiffModel(opt, device) 
    #model = LongMriFreezeModel(opt, device) 
    #model = MultiMripntfreezeModel(opt, device) 
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        for i in range(batches_per_epoch):
            ...
            rec_loss_1 += g_losses['seg_loss_1']
            rec_loss_2 += g_losses['seg_loss_2']
            #grad_loss += g_losses['grad_loss']
            #normal_loss += g_losses['normal_loss']
            if opt.latent_code_regularization:
                latent_loss += g_losses['latent_loss']
        model.update_learning_rate(epoch)
        logs = {}
        logs['rec_loss_1'] = rec_loss_1 / batches_per_epoch
        logs['rec_loss_2'] = rec_loss_2 / batches_per_epoch
        #logs['mnf_loss'] = rec_loss / batches_per_epoch
        #logs['grad_loss'] = grad_loss / batches_per_epoch
        #logs['normal_loss'] = normal_loss / batches_per_epoch
        if opt.latent_code_regularization:
        ...
        if val_dataloader is not None:
            #img_gt, seg_gt, val_output, val_output_diff, val_seg_output, val_rec_loss_1, val_rec_loss_2,val_fm_loss, val_score \
            #        = model.run_evaluation_during_trainingATT(val_dataloader, (patch_depth,patch_height,patch_width))
            img_gt, seg_gt, val_output,  val_seg_output, val_rec_loss_1, val_rec_loss_2,val_fm_loss, val_score \
                    = model.run_evaluation_during_trainingATT(val_dataloader, (patch_depth,patch_height,patch_width))
            logs['val_rec_loss_1'] = val_rec_loss_1
            logs['val_rec_loss_2'] = val_rec_loss_2
            logs['val_fm_loss'] = val_fm_loss
            logs['val_score'] = val_score
            pred_image = val_output.squeeze().unsqueeze(1)
            logs['synthesized_sdf'] = wandb.Image(pred_image)
            
            #pred_diff = val_output_diff.squeeze().unsqueeze(1)
            #logs['synthesized_diff'] = wandb.Image(pred_diff)
            
            pred_seg = val_seg_output.squeeze().unsqueeze(1)
            logs['synthesized_seg'] = wandb.Image(pred_seg)
            
            real_image = img_gt.squeeze().unsqueeze(1)
            logs['real_image'] = wandb.Image(real_image)
            real_seg = seg_gt.squeeze().unsqueeze(1)
            logs['real_seg'] = wandb.Image(real_seg)
        wandb.log(logs)
