...
def dice_coeff3D(input, target, reduce_batch_first: bool = False, epsilon=1e-6):
...
def load_pretrained_network(net, label, epoch, opt,strict):
    save_filename = '%s_net_%s.pth' % (epoch, label)
    #save_dir = os.path.join(opt.pretrained, opt.name)
    save_path = os.path.join(opt.pretrained, save_filename)
    print(save_path)
    weights = torch.load(save_path)
