...
if __name__=='__main__':
    ...
    for patient in patientlist:
        ...
        for i in range(len(scans)-2):
            test_sequence = scans[0:2]
            test_sequence.append(scans[i+2])
            print(test_sequence)
            scan_save_dir = os.path.join(save_dir, patient+'_'+scans[i+2])
            ...
            #print(pred_seg.shape)
                for j in range(pred_seg.shape[1]):
                    ...
                    save_name = os.path.join(scan_save_dir, patient + "_" + test_sequence[j+1] + ".nii.gz")
                    nib.save(img_to_save,save_name)
            
                    mask_save_name = os.path.join(scan_save_dir, patient + "_" +test_sequence[j+1] +"_seg.nii.gz")
